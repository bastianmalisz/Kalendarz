 ...
 function drawMonth(nazwaMiesiaca, dzienTygodnia, dniTygodniaNazwa, day, rok, dniMiesiecyZwykly, nextMonth) 
 {
    clear();
     ...
     for (let i = 1; i < dniMiesiecyZwykly[miesiac] + day; i++){
         ...
         if(i === dzis && miesiac == obecnyCzas.getMonth()){document.querySelector('.test').innerHTML += '<div id="day_' + (i - day + 1) + '" class="box day' + (i - day + 1) + ' ' + nazwyMiesiecy[miesiac + nextMonth] + (i - day + 1) + rok +" " + (i - day + 1)+" "+"today"+ ' ">' + (i - day + 1) + '</div>';}   else{
...
function hasEvent(){
 ...
 {
     if (i < 10) { i = "0" + i };  // dodaj zero jesli numer < 10
     return i;
 }
...
let i,ls = localStorage,l = ls.length,key,objectKey = [];
